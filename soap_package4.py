...
def g(rcut,n_max,r):
    ...
    S = np.empty([n_max,n_max]) #matrix containing overlap integrals of n and n'
...
def soap_desc(atoms,rcut,l_max,n_max,atom_sigma):
    ...
    GY = np.empty((len(r_sph),n_max,l_max+1),dtype="object")           
        ...
        for j in range(n_max):
            for k in range(l_max+1):
                ...
                for z in range(2*k+1): 
                    gy.append(Gn[i][j]*Ylm[i][k][z])
                GY[i][j][k] = np.array(gy)

    #inverse of the multiple of gn(r) and Ylm(r)        
    G = np.conj(GY).T

    P = np.empty(n_atom, dtype = "object")
    for f in range(n_atom):
        #c_atom = atom_pos[f] #position of the central atom in cartesian
        d = atoms.get_distances(np.arange(n_atom),f) #calculating the distance between central atom and all other atoms
        D = atoms.get_distances(np.arange(n_atom),f, vector = True) #distance vectors
        ...
        for i in range(len(r_cart)):
            ...
            for j in range(len(Di)):
                gaussian.append(Gaussian(r_cart[i],Di[j],a)) 
        ...
        for i in range(n_max):
            for j in range(l_max+1):
                c_nlm[i][j]=np.array(np.matmul(atom_neigh_den,G[j][i]))
